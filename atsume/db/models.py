...
class ModelMetaclass(OrmarModelMetaclass):
    def __new__(mcs: OrmarModelMetaclass, name: str, bases: typing.Any, attrs: dict):
        ...
        if not hasattr(meta, "tablename"):
            meta.tablename = f"{meta.component_config.name.lower()}_{meta.get_plural_name()}"

        if not hasattr(meta, "database"):
            meta.database = database.database

        if not hasattr(meta, "metadata"):
            meta.metadata = database.metadata
